...
df
fit1 = lm(log(Life.expectancy) ~ Adult.Mortality+infant.deaths+Alcohol+percentage.expenditure+Hepatitis.B+Measles+
...
fit4 = lm(formula = Life.expectancy ~ Adult.Mortality +  
...
qqnorm(residuals(fit_final), pch=19)
qqline(residuals(fit_final))
summary(coch)
shapiro.test(residuals(fit_final))

par(mfrow = c(2,2))
hist(df$Life.expectancy)
hist(residuals(fit_final))
